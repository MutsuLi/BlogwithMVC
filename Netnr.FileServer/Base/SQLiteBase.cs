using SQLite;
using System;
using Blog.FileServer.Model;
using System.Collections.Generic;
using Blog.Common;

namespace Blog.FileServer.Base
{
    /// <summary>
    /// 数据库操作
    /// </summary>
    public class SQLiteBase
    {
        ...
        public static ActionResultVM CreateApp()
        {
            ...
            try
            {
                ...
                var mo = new SysKey()
                {
                    SkAppId = UniqueTo.LongId().ToString(),
                    SkAppKey = UniqueTo.LongId().ToString() + UniqueTo.LongId().ToString(),
                    SkCreateTime = DateTime.Now,
                    SkName = "默认",
                    SkToken = CalcTo.MD5(UniqueTo.LongId().ToString()),
        ...
        public static ActionResultVM GetToken(string AppId, string AppKey)
        {
            ...
            try
            {
                ...
                if (sk != null)
                {
                    sk.SkToken = CalcTo.MD5(UniqueTo.LongId().ToString());
